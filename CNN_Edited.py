...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 25 12:23:28 2019
...
PathSpectogramFolder='/Volumes/chenchen/CNNs-on-CHB-MIT_edited/Results/dataset/spectograms'
OutputPath='/Volumes/chenchen/CNNs-on-CHB-MIT_edited/Results'
OutputPathModels='/Volumes/chenchen/CNNs-on-CHB-MIT_edited/Results/models'
...
def loadSpectogramData(indexPat):
    ...
    line=f.readline()
    line=f.readline()#read the number of spectograms. I do not save it since I do not need it
...
def generate_arrays_for_training(indexPat, paths, start=0, end=100):
    while True:
        ...
        for i in range(from_, int(to_)):
            ...
            x=np.array([x])
            print('x after load is: ', x.shape)
            x=x[:,:,:,:,::100];
            print('x after downsample is: ', x.shape)
            x=x.swapaxes(0,1)
            print('x after swaoaxes is: ', x.shape)
            if('P' in f):
